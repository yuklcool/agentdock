...
这套部署只拉取预构建镜像，不需要 Git、Node.js、pip 或本地编译。需要 Linux amd64、Docker Engine、Docker Compose v2 和 Python 3（仅用于首次生成配置）。镜像统一发布到 `ghcr.io/yuklcool/agentdock`，版本为 `0.3.1`。
...
mkdir agentdock
cd agentdock
curl -fL https://github.com/yuklcool/agentdock/releases/download/v0.3.1/agentdock-deploy.tar.gz -o agentdock-deploy.tar.gz
...
所有镜像均使用 `ghcr.io/yuklcool/agentdock/<名称>:0.3.1`：
