...
创建实例后，`agent_id` 即平台返回的容器/实例 id。绑定实例可通过所属用户、管理员登录会话或本工作空间的可信服务 API Key 访问。
以下服务端示例使用工作空间 API Key，目标可为本空间内的公共或用户绑定实例；密钥表示空间级服务身份，不携带用户身份：
...
管理员或工作空间 Owner 在 **Fleet → New** 创建实例时，可通过 **绑定用户（可选）** 选择当前工作空间中的有效用户。提交后实例为该用户的私有实例，用户登录后可直接在实例列表中看到并使用；其他普通用户无法访问，工作空间管理员仍可管理。

下拉列表显示姓名和邮箱，不包含已禁用的用户、无有效成员关系的用户或 Staff 平台账号。后台同样执行校验，不能通过伪造请求绕过。普通用户不显示该字段，私有实例仍自动归属自己。管理员默认“不绑定”，创建公共容器；普通成员默认绑定自己。

API：`POST /v1/containers`，管理员可传入 `visibility: "private"` 和 `owner_user_id`。共享实例不能同时指定归属用户。每个工作空间内一个用户最多绑定一个非 destroyed 容器，数据库唯一索引和并发名额预留共同保证；审计保留实际操作者和目标归属人。管理员实例列表显示绑定用户，成员失效后无法解析姓名时回退展示用户 ID。

该功能不自动迁移已有实例的归属；注册用户本身也不会立即创建实例。升级平台到 0.3.2 即可使用，已有运行容器无需为本功能重建。

### 配额与审计页面（0.3.3）

在 Settings → 配额与审计中，管理员/Owner/Staff 可查看绑定容器上限，并分组设置工作空间每日限额与单用户每日限额。每日预算留空或填 0 表示不限额；每用户在当前工作空间的绑定容器上限固定为 1。展开“每日限额如何计算？”可查看 UTC 重置和 Token 预留说明。
...
容器自身就是实例，以 `owner_user_id` 绑定用户，不增加 Agent 表或额外绑定表。管理员创建时显式传 `owner_user_id: null` 表示不绑定，也可省略此字段；可创建多个公共容器。传具体用户 ID 时须为当前工作空间有效成员。重复绑定返回 409 `user_container_already_bound`，暂停及归档不释放名额。

可信服务端使用 Workspace API Key：

```bash
# 默认返回未绑定容器
curl "$AGENTDOCK_URL/v1/containers" -H "Authorization: Bearer $WORKSPACE_KEY"
# 返回该用户的唯一有效绑定；没有则 containers 为空数组
curl "$AGENTDOCK_URL/v1/containers?owner_user_id=$USER_ID" \
  -H "Authorization: Bearer $WORKSPACE_KEY"
# 对查到的容器发送任务；会话和事件沿用现有接口
curl "$AGENTDOCK_URL/v1/containers/$CONTAINER_ID/tasks" \
